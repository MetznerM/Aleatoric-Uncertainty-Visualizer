...
# -*- coding: utf-8 -*-
"""
Created on Thu Nov 16 03:04:57 2023
...
model = tf.keras.models.Sequential([
    ...
    layers.Dense(16, activation='relu'), #32
...
def calculate_bic(data, component_range):
    ...
    for n_components in component_range:
        gmm = GaussianMixture(n_components=n_components, covariance_type='diag', random_state=SEED)
        gmm.fit(data)
...
gmm = GaussianMixture(n_components=n_components, warm_start=True, covariance_type='diag', random_state=SEED)
...
for i in range(n_components):
    ...
    for j in range(activation_before_sm.shape[1]):
        ...
        # Berechnen der PDF für die Komponente
        y = norm.pdf(x, mean[j], np.sqrt(gmm.covariances_[i][j])) * weight
        total_curve += y
...
for dim in range(activation_before_sm.shape[1]):
    ...
    gmm = GaussianMixture(n_components=5) #3
    ...
    plt.plot(x, y, label=f'GMM Fit Dim {dim+1}') #, color=sns.color_palette("plasma", 5)[dim]
    plt.title(f'GMM Fit Dim {dim+1}')
    plt.show()
