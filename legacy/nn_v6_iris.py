...
from scipy.stats import gaussian_kde, norm, multivariate_normal, entropy, skew, kurtosis
...
# (Semi-)Vollständige Betrachtung
uncertain_attr = [0, 1, 2, 3, 4, 5] # uncertain !!!nodes!!!
# Betrachtung PDF in 3D
#uncertain_attr = [0, 3]
#uncertain_attr = [2, 6]
...
for i, activation in enumerate(activations):
    if i == (len(activations)-2):
        if dataset == "heart" or dataset == "iris":
...
samples = gmm.sample(10000)[0].reshape(-1)
...
#plt.plot(x, gmm_cov*norm.pdf(x, gmm_mean, gmm_cov), color="blue", label="Normalverteilung")
#plt.plot(x, norm.pdf(x, gmm_mean, gmm_cov), color="aqua", label="GMM-Norm")
...
if len(uncertain_attr) == 2:
    ...
    ax.plot_surface(x, y, z, cmap='plasma', alpha=1, label=f'Komponente {i+1}')
