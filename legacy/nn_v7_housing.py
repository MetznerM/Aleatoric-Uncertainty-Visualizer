...
# -*- coding: utf-8 -*-
"""
Created on Sun Nov 19 19:26:42 2023
...
# -*- coding: utf-8 -*-
"""
Created on Thu Nov 16 16:18:03 2023
...
# -*- coding: utf-8 -*-
"""
Created on Thu Nov 16 03:02:32 2023
...
#uncertain_attr = [0, 3] # uncertain nodes
uncertain_attr = [0] # 2, 3
...
if dataset == "housing":
    selected_features = ["Distance", "Rooms", "Propertycount", "Bathroom", "Landsize", "Longtitude", "Lattitude", "Suburb", "YearBuilt"]
    X = X[selected_features]
    
plt_rows = int(np.ceil(np.sqrt(len(X.columns))))
plt_cols = int(np.ceil(len(X.columns)/plt_rows))
...
use_model = True
model_path = "saved_model/model_housing-v7.keras"
if os.path.exists(model_path) and use_model:
    print("load model...")
    ...
    model.compile(optimizer='adam', loss='mean_squared_error', metrics=['mse']) # regression
...
sim_data = sc.transform(sim_data)
pred_probs = model.predict(sim_data, verbose=False)
print("shape", pred_probs.shape)
pred_probs = sc_y.inverse_transform(pred_probs).ravel()
...
for i, activation in enumerate(activations):
    if i == (len(activations)-2):
        if dataset == "heart" or dataset == "iris" or dataset == "housing":
            ...
            if use_gt:
                ax.set_title("KDE vor Output-Layer (Ground Truth)")
            else:
                ax.set_title("KDE vor Output-Layer")
...
if use_gt:
    ax.set_title("Histogramm/KDE der Modellvorhersagen (Ground Truth)")
else:
    ax.set_title("Histogramm/KDE der Modellvorhersagen")
...
def calculate_bic(data, component_range):
    ...
    for n_components in component_range:
        print("comp:", n_components)
        gmm = GaussianMixture(n_components=n_components, warm_start=True, covariance_type='full', random_state=SEED)
...
bics = calculate_bic(pred_probs, component_range)
fig = plt.figure()
...
for dim in range(1):
    ...
    gmm = GaussianMixture(n_components=3) #3
    gmm.fit(pred_probs.reshape(-1,1))
    #gmm_models.append(gmm)
    
    bic = gmm.bic(pred_probs.reshape(-1,1))
...
bic = gmm.bic(pred_probs.reshape(-1, 1))
aic = gmm.aic(pred_probs.reshape(-1, 1))
...
x = np.linspace(-70000, 7000000, 2000)
...
for i in range(n_components):
    ...
    #if i in [0, 1, 2, 3, 4, 6, 9]: #2.=5.? 4.=7.->#2? 8. weg 9.=>#1 #10&#11 weg
...
if len(uncertain_attr) == 2:
    ...
    x = np.linspace(-7000, 12000, 2000)
    y = np.linspace(-7000, 12000, 2000)
    x, y = np.meshgrid(x, y)
